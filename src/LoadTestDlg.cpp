...
void LoadTestDlg::CreateWidget()
{
    ...
    testTable_->setColumnCount(5);
    testTable_->setColumnWidth(0, 80);
    testTable_->setColumnWidth(1, 100);
    testTable_->setColumnWidth(2, 100);
    testTable_->setColumnWidth(3, 100);
    testTable_->setColumnWidth(4, 200);
    QStringList strheadList;
    strheadList<<"序号"<<"测试名称"<<"测试人"<<"被测试人"<<"时间";
...
void LoadTestDlg::LoadTestList()
{
   ...
   for (int i=0; it != infoList.end(); ++it)
   {
       if (m_strUserName == it->sUser && it->nIndex != m_nAmsweringSaveIndex)
       {
           ...
           testTable_->setItem(i, 2, new QTableWidgetItem(it->sTesterName));
           testTable_->setItem(i, 3, new QTableWidgetItem(it->sAnswerName));
           testTable_->setItem(i, 4, new QTableWidgetItem(it->sDateTime));
           ++i;
       }
   }
}
...
void LoadTestDlg::ClickDel()
{
    ...
    {
        QMessageBox::about(this, "删除答案", "删除答案成功");
    }
