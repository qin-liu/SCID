...
{
	//size_t stringSize; //mdy by xujianhui	
	...
	//mdy by xujianhui
	if (size == 0) size = stringSize;
	if (size>0) bytesRead += ContinueRead(data+2, size);
	else bytesRead = 3;
	//mdy over
...
size_t LargeString::ContinueRead(const char* data, size_t size)
{
	...
	if (unicode_ & 1)
	{
		...
		size_t npos = 1;
		if (richtext_ && wname_.size() == 0) npos += 2;
		if (phonetic_ && wname_.size() == 0) npos += 4;
		...
		if (richtext_ && wname_.size() == stringSize) npos += 4*richtext_;
		if (phonetic_ && wname_.size() == stringSize) npos += phonetic_;
		return npos;
	}
	else
	{
		...
		size_t npos = 1;
		if (richtext_ && name_.size() == 0) npos += 2;
		if (phonetic_ && name_.size() == 0) npos += 4;
		...
		if (richtext_ && name_.size() == stringSize) npos += 4*richtext_;
		if (phonetic_ && name_.size() == stringSize) npos += phonetic_;
		return npos;
	}
}
...
size_t Workbook::SharedStringTable::Read(const char* data)
{
	...
	else
	{
		...
		for (int i=0, c=0; i<uniqueStringsTotal_; ++i)
		{
			...
			if (c >= (int)maxContinue || npos+stringSize*multiplier+3+nExtSize <= continueIndices_[c])
			{
			...
			else
			{
				...
				if (size > 0)  
				{
					...
					npos += strings_[i].Read(&*(data_.begin())+npos, size); // add by xujianhui
					stringSize -= size;
					size = 0;
				...
				while (c<(int)maxContinue && npos+stringSize*multiplier+1>continueIndices_[c])
				{
					...
					npos += strings_[i].ContinueRead(&*(data_.begin())+npos, dataSize); // add by xujianhui
					//stringSize -= dataSize + 1;
					stringSize -= dataSize; // add by xujianhui
					++c;
				};
				if (stringSize>0)
				{
...
size_t Worksheet::CellTable::RowBlock::CellBlock::Formula::Read(const char* data)
{
	...
	LittleEndian::Read(data, code, offset, 2);
	if (code == CODE::eSTRING) string_.Read(data+offset);
	return RecordSize();
}	
...
Worksheet::CellTable::RowBlock::CellBlock::Formula::String::String() : Record()
	{code_ = CODE::eSTRING; dataSize_ = 0; recordSize_ = 4;}
...
{
	if (type_ == eSTRING) return str_.size() - 1;
